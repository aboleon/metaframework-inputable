...
class BladeComponentsRenderTest extends TestCase
{
    ...
    public function test_textarea_component_defaults_to_text_mode_and_renders_validation_error(): void
    {
        ...
        $this->assertStringNotContainsString('data-mfw-inputable-editor-mode="html"', $html);
        $this->assertStringNotContainsString('tinymce.min.js', $html);
        $this->assertStringNotContainsString('cherry-markdown.min.js', $html);
        $this->assertStringNotContainsString('data-mfw-inputable-content-type-radio', $html);
    }

    public function test_textarea_component_markdown_mode_renders_cherry_markup_and_assets(): void
    {
        $html = Blade::render(
            '<x-mfw-inputable::textarea name="notes" mode="markdown" :height="120" />@stack("js")'
        );
    ...
    public function test_textarea_component_html_mode_activates_tinymce_without_rendering_radios(): void
    {
        $html = Blade::render(
            '<x-mfw-inputable::textarea name="notes" mode="html" class="extended" />@stack("js")'
        );

        $this->assertStringContainsString('data-mfw-inputable-editor-mode="html"', $html);
        $this->assertStringContainsString('data-mfw-inputable-tinymce-preset="extended"', $html);
    ...
    public function test_textarea_component_invalid_mode_falls_back_to_text_mode(): void
    {
        $html = Blade::render(
            '<x-mfw-inputable::textarea name="notes" mode="legacy" class="extended" />@stack("js")'
        );
    ...
    public function test_textarea_component_content_type_html_string_maps_to_html_mode_without_radios(): void
    {
        $html = Blade::render(
    ...
    public function test_textarea_component_content_type_true_infers_html_when_editor_class_exists(): void
    {
        $html = Blade::render(
    ...
    public function test_textarea_component_does_not_parse_mode_or_type_from_params(): void
    {
        $html = Blade::render(
            '<x-mfw-inputable::textarea name="notes" :params="$params" />@stack("js")',
            ['params' => ['type' => 'text', 'mode' => 'markdown', 'data-x' => '1']]
        );

        $this->assertStringContainsString('data-x="1"', $html);
        $this->assertStringContainsString('type="text"', $html);
        $this->assertStringContainsString('mode="markdown"', $html);
        $this->assertStringNotContainsString('data-mfw-inputable-cherry="1"', $html);
        $this->assertStringNotContainsString('vendor/mfw-inputable/components/textarea-cherry.js', $html);
    }

    public function test_textarea_component_readonly_skips_editor_initialization(): void
    {
        $markdownHtml = Blade::render(
            '<x-mfw-inputable::textarea name="notes" mode="markdown" readonly />@stack("js")'
        );
        $htmlModeHtml = Blade::render(
            '<x-mfw-inputable::textarea name="body" mode="html" readonly />@stack("js")'
        );

        $this->assertStringContainsString('readonly', $markdownHtml);
        $this->assertStringNotContainsString('data-mfw-inputable-cherry="1"', $markdownHtml);
        $this->assertStringNotContainsString('cherry-markdown.min.js', $markdownHtml);
        $this->assertStringNotContainsString('data-mfw-inputable-editor-mode="html"', $htmlModeHtml);
        $this->assertStringNotContainsString('tinymce.min.js', $htmlModeHtml);
    }
